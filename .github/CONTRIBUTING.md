...
- [Slack](https://vmwarecode.slack.com/messages/kov-engine): This is the primary community channel. If you don't have an @vmware.com or @emc.com email, please sign up at https://code.vmware.com/slack to get a Slack invite.

- [Gitter](https://gitter.im/vmware/kov): Gitter is monitored but go to Slack if you need a response quickly.

## Getting started

First, fork the repository on GitHub to your personal account.

Note that _GOPATH_ can be any directory, the example below uses _$HOME/kov_.
Change _$USER_ below to your GitHub username.

``` shell
export GOPATH=$HOME/kov
mkdir -p $GOPATH/src/github.com/vmware
go get github.com/vmware/kov
cd $GOPATH/src/github.com/vmware/kov
git config push.default nothing # anything to avoid pushing to vmware/kov by default
git remote rename origin vmware
git remote add $USER git@github.com:$USER/kov.git
git fetch $USER
```
...
- Submit a pull request to vmware/kov.
...
[dronekov]:https://ci.vmware.run/vmware/kov
[e2edronekov]:https://e2e.ci.vmware.run/vmware/kov
...
You can run the tests locally before making a PR or view the Drone build results for [unit tests][dronekov]
and [integration tests][e2edronekov].
...
browser plugin installed, click on the [Boards](https://github.com/vmware/kov/issues#boards) tab to open the Zenhub task
board.
...
* lib - common library packages that are tightly coupled to vmware/kov
* pkg - packages that are not tightly coupled to vmware/kov and could be usefully consumed in other projects. There is still some sanitization to do here.
